...
def bind_claim_values(results,claim,periods,contexts=()):
    ...
    if re.search(r'\b(?:increas\w*|decreas\w*|ris\w*|rose|grew|grow\w*|fell|fall\w*|declin\w*)\s+by\b',text,re.I):
        if not any(any(token in col.lower() for token in ['difference','change','minus','delta']) for col in columns):
            raise QueryBlocked('An increase/decrease BY an amount must cite a calculated difference or change, not an ending value. Use rose TO for the ending value or cite the actual difference.')
