...
The live explanation reviewer also sometimes copied rendered numbers into its revised draft. The reviewer now receives the editable placeholder draft separately from the rendered answer, with explicit instructions to preserve citations. Unsupported numeric literals remain blocked. Query validation failures are visible in the evidence panel even when no results were returned.

Final verification: all 88 automated tests and nine subtests passed; all nine checks on the deployed validation page passed. After deployment, the actual GPT-4.1 mini chat answered the original Sarah question with AUD 1,410 total, AUD 1,380 service and AUD 30 retail. That live response used read-only SQL against saved cleaned version `0cdcac31-2d93-4e6c-b156-c3dc092195b0`; its evidence includes the exact version and queries. The explicit revenue module is covered separately by automated tests. Live model routing can still choose permitted SQL, and an invalid query or unsupported explanation is withheld rather than treated as verified.

After an app restart, both pages recovered the same saved version. Supabase retained one original raw batch, one clean version and one seed audit event, without duplicate imports. The three ambiguous fixture issues remain available for the owner to review; no owner decision or sale was fabricated on the active dataset.
