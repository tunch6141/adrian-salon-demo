...
def setting(key):
    ...
    except FileNotFoundError:return os.environ.get(key,'')
...
def render_chat(intake):
    ...
    text=st.chat_input('Reply to the question or describe a business event',key='p1_chat_input',max_chars=2000)
    if text:
        ...
        try:
            ...
            if parsed['intent']=='transaction':
                ...
                st.session_state['p1_pending']={'kind':'context','text':text,'selected':selected}
                answer='I can record your exact wording as owner-provided context. Review the entity and dates below. This will not change bookings, revenue or capacity.'
            elif parsed['intent']=='not_configured':answer='AI interpretation is not configured. Choose the correction directly, or add your existing model settings.'
            elif parsed['intent']=='uncertain':answer='Please state the name/ID or unit cost in AUD excluding GST explicitly. If unsure, leave this issue unresolved.'
            else:answer='This chat handles data clarification and business events. Full commercial investigation is not connected here yet.'
        except Exception as exc:answer=str(exc) if isinstance(exc,ValueError) else 'The AI request did not complete. Nothing was changed. Retry or choose the correction directly.'
        history.append({'role':'assistant','content':answer});st.rerun()
    if q and (mode=='Choose the correction directly' or not(key and model)):
        if q['kind']=='cost':
            value=st.text_input('Unit cost in AUD excluding GST',key='p1_cost:'+selected);evidence='Owner entered AUD '+value+' excluding GST'
        else:
            opts={o['value']:o['label'] for o in q['options']}
            value=st.selectbox('Confirmed identity',['']+list(opts),format_func=lambda x:opts.get(x,'Select an identity'),key='p1_choice:'+selected);evidence='Owner selected '+opts.get(value,'')+' '+value
        if st.button('Review proposed correction',key='p1_review'):
            try:st.session_state['p1_pending']={'kind':'correction','draft':proposed(q,value,evidence),'selected':selected};st.rerun()
            except ValueError as exc:st.error(str(exc))
    pending=st.session_state['p1_pending']
    if pending and pending['selected']==selected:
        if pending['kind']=='correction':
            st.write('**Proposed correction:** '+pending['draft']['summary'])
            st.caption('Raw source files remain unchanged. The approved decision is reusable through the checkpoint.')
            if st.button('Confirm correction',type='primary',key='p1_confirm'):
                try:
                    updated=st.session_state['p1_decisions']+[decision(pending['draft'],owner)];Intake(raw,updated)
            ...
            start=st.date_input('Event start date',value=None,key='p1_event_start');end=st.date_input('Event end date',value=None,key='p1_event_end')
            if st.button('Confirm business context',type='primary',key='p1_confirm_context'):
                try:
                    if not start or not end:raise ValueError('Choose both event dates.')
                    st.session_state['p1_context'].append(context_note(pending['text'],entity,start,end,owner));st.session_state['p1_pending']=None
    ...
    with st.expander('Restore an owner-review checkpoint'):
        upload=st.file_uploader('Checkpoint JSON',type=['json'],key='p1_restore_file')
        if upload:
            try:
