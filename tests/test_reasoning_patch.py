...
def test_uncited_aside_does_not_suppress_supported_core_answer(db):
    ...
    a=Answer(claims=[dict(text='Revenue was [[0]].',evidence=[dict(result=0,row=0,column='service_revenue_aud',format='money')],context_ids=[]),
        dict(text='Unsubstantiated aside.',evidence=[],context_ids=[])],investigation='',recommendation='',measurement='',missing_information='',chart=dict(kind='bar',result=99,x='staff_name',y='revenue'))
    with patch('analyst_ai.structured',side_effect=[p,a,Review(approved=True,issues=[])]) as calls:
        result=investigate(None,'gpt-4.1-mini',db,'Sarah on September seventeenth?',[],ContextStore())
    assert result['status']=='answered'
    ...
    assert result['answer']['chart']['kind']=='none'
    assert 'Optional chart omitted' in result['execution_notes'][0]
...
def test_staff_narration_supports_evidence_bound_numeric_parts():
    from types import SimpleNamespace
    from unittest.mock import MagicMock
    from pydantic import ValidationError
    from analyst_ai import structured
    wire=dict(claims=[dict(parts=[dict(kind='text',text='Completed service hours: '),dict(kind='value',citation=dict(result=0,row=0,column='completed_service_hours',format='plain')),dict(kind='text',text='.')],evidence=[],context_ids=[])],
    ...
    assert answer.claims[0].text=='Completed service hours: [[0]].'
    assert bind_claim_values([dict(rows=[dict(completed_service_hours=18)])],answer.claims[0].model_dump(),['',''])=='Completed service hours: 18.'
    schema=client.responses.parse.call_args.kwargs['text_format']
    assert 'obsolete placeholder instructions' not in client.responses.parse.call_args.kwargs['instructions']
    invalid=deepcopy(wire);invalid['claims'][0]['parts'][0]['text']='Hours decreased by [[0]].'
    with pytest.raises(ValidationError):schema.model_validate(invalid)
